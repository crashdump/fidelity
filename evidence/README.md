...
| `identity` | `integrity.platform_trust` | Linux | Debian, glibc, in a container, 2026-08-13 | the tier reports `Unsupported`. The ioctl runs and the kernel answers that this image carries no fs-verity, which is what an ordinary filesystem reports. | none. An `Unsupported` tier creates no finding, so it has no hostile control. |
| `identity` | `integrity.expected_identity` | Linux | Debian, glibc, in a container, 2026-08-13 | the running image matches the digest of its own content, read through `/proc/self/exe` | a pinned digest of another value, which reports, `High`. No repackaged-binary control exists yet, and the gaps below name it. |
...
| A filesystem with fs-verity | The `Accepted` half of Linux `integrity.platform_trust`. The container runs on overlayfs, which supports no verity, so only the absent path has ever run. An ext4 image with verity enabled would exercise the other arm. |
