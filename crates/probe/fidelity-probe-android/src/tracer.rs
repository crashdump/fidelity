//! The tracer capability on Android.
//!
//! The capability needs no JVM handle and no Java call. Android keeps the
//! Linux process filesystem, so the kernel answers the question directly.
...
impl Tracer for AndroidEnvironment {
...
mod tests {
    ...
    fn a_clean_process_reports_no_tracer() {
        // The clean control. The hostile control attaches `lldb-server` on a
        // device, because a test runner that traces itself cannot also be the
        // clean control.
        assert_eq!(
            AndroidEnvironment::new().tracer_state(),
