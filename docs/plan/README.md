...
| Runtime | One immutable production runtime per process that runs until the process stops, with no public stop; a synchronous initial scan of the cheap detectors, then the worker applies its actions and runs the rest | [Runtime](03-runtime-and-api.md), [ADR-0005](../adr/0005-library-owned-runtime.md) |
...
Performance figures come from the first vertical slice, against the rule in
[state and budgets](07-state-and-budgets.md). Concrete detectors, evidence variants, and
per-platform baseline mechanics need implementation research and empirical testing. They are
engineering work, not open product choices.
